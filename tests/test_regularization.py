...
def test_ElasticNetRegularization(model_config):
    ...
    regularizer_elastic_l1 = ElasticNetRegularization(regularization_rate=0.01, alpha=1, biases=False)
    regularizer_elastic_l2 = ElasticNetRegularization(regularization_rate=0.01, alpha=0, biases=False)
    
    obtained_term_l1 = regularizer_l1(model.named_parameters(), device=dev_).item()
    obtained_term_l2 = regularizer_l2(model.named_parameters(), device=dev_).item()
    obtained_term_elastic_l1 = regularizer_elastic_l1(model.named_parameters(), device=dev_).item()
    obtained_term_elastic_l2 = regularizer_elastic_l2(model.named_parameters(), device=dev_).item()
    ...
    # ---------------------
    elastic = ElasticNetRegularization(regularization_rate=0.01, alpha=0.5, biases=False)
